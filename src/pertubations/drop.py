...
def drop_rand_verb(sentence, words, pos_tags):
    nn_indices = [i for i, tag in enumerate(pos_tags) if tag == "V_VM" or tag == "V__VM__VF" or tag == "V__VM__VNF" or tag == "V__VM__VINF" or tag == "V__VM__VNG" or tag =="V_VAUX"]
    if nn_indices:
        nn_index_to_drop = random.choice(nn_indices)
        updated_words = [word for i, word in enumerate(words) if i != nn_index_to_drop]
        updated_sentence = " ".join(updated_words)
        return updated_sentence.strip()
    else:
        return sentence

def drop_rand_noun(sentence, words, pos_tags):
    nn_indices = [i for i, tag in enumerate(pos_tags) if tag == "N_NN" or tag == "N_NNP" or tag == "N_NST"]
    if nn_indices:
        nn_index_to_drop = random.choice(nn_indices)
        updated_words = [word for i, word in enumerate(words) if i != nn_index_to_drop]
        updated_sentence = " ".join(updated_words)
...
def drop_all_verbs(sentence, words, pos_tags):
    nn_indices = [i for i, tag in enumerate(pos_tags) if tag == "V_VM" or tag == "V__VM__VF" or tag == "V__VM__VNF" or tag == "V__VM__VINF" or tag == "V__VM__VNG" or tag =="V_VAUX"]
    if nn_indices:
        updated_words = [word for i, word in enumerate(words) if i not in nn_indices]
        updated_sentence = " ".join(updated_words)
        return updated_sentence.strip()
    else:
        return sentence

def drop_all_nouns(sentence, words, pos_tags):
    nn_indices = [i for i, tag in enumerate(pos_tags) if tag == "N_NN" or tag == "N_NNP" or tag == "N_NST"]
    if nn_indices:
        updated_words = [word for i, word in enumerate(words) if i not in nn_indices]
        updated_sentence = " ".join(updated_words)
...
def dropall(sentence, words, pos_tags):
    nn_indices = [i for i, tag in enumerate(pos_tags) if tag == "V_VM" or tag == "V__VM__VF" or tag == "V__VM__VNF" or tag == "V__VM__VINF" or tag == "V__VM__VNG" or tag =="V_VAUX" or tag == "N_NN" or tag == "N_NNP" or tag == "N_NST"]
    if nn_indices:
        updated_words = [word for i, word in enumerate(words) if i not in nn_indices]
        updated_sentence = " ".join(updated_words)
        return updated_sentence.strip()
...
def dropVB(sentence,type):
    ...
    # print(text)    
    if type == "dropallboth":
        updated_sentence = dropall(text,words,labels)
    elif type == "dropallNN":
        updated_sentence = drop_all_nouns(text,words,labels)
    elif type == "dropallVB":
        updated_sentence = drop_all_verbs(text,words,labels)
    elif type == "droprandNN":
        updated_sentence = drop_rand_noun(text,words,labels)
    elif type == "droprandVB":
        updated_sentence = drop_rand_verb(text,words,labels)
...
def main():
    ...
    if not os.path.isdir(args.inp):
        ...
        for sentence in ssf_doc.nodeList:
            ...
            drop.append(dropVB(sentence,args.out[15:-4]))
            # exit(0)
